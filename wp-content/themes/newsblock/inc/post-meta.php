...
if ( ! function_exists( 'csco_block_post_meta' ) ) {
	...
	function csco_block_post_meta( $settings, $meta, $echo = true, $compact = false ) {
		...
		if ( isset( $settings[ $prefix . 'showMetaCategory' ] ) && $settings[ $prefix . 'showMetaCategory' ] ) {
			$allowed[] = 'category';
		}

		if ( isset( $settings[ $prefix . 'showMetaAuthor' ] ) && $settings[ $prefix . 'showMetaAuthor' ] ) {
			$allowed[] = 'author';
		}

		if ( isset( $settings[ $prefix . 'showMetaDate' ] ) && $settings[ $prefix . 'showMetaDate' ] ) {
			$allowed[] = 'date';
		}

		if ( isset( $settings[ $prefix . 'showMetaComments' ] ) && $settings[ $prefix . 'showMetaComments' ] ) {
			$allowed[] = 'comments';
		}

		if ( isset( $settings[ $prefix . 'showMetaViews' ] ) && $settings[ $prefix . 'showMetaViews' ] ) {
			$allowed[] = 'views';
		}

		if ( isset( $settings[ $prefix . 'showMetaReadingTime' ] ) && $settings[ $prefix . 'showMetaReadingTime' ] ) {
			$allowed[] = 'reading_time';
		}

		if ( isset( $settings[ $prefix . 'showMetaShares' ] ) && $settings[ $prefix . 'showMetaShares' ] ) {
			$allowed[] = 'shares';
		}

		if ( isset( $settings[ $prefix . 'metaCompact' ] ) && $settings[ $prefix . 'metaCompact' ] ) {
			$compact = true;
		}
...
if ( ! function_exists( 'csco_get_meta_date' ) ) {
	...
	function csco_get_meta_date( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( get_the_time( 'm/d/Y H:i' ) !== get_the_modified_time( 'm/d/Y H:i' ) ) {
	...
	function csco_get_meta_author( $tag = 'div', $compact = true, $settings = array() ) {
		...
		if ( $authors ) {
			...
			foreach ( $authors as & $author ) {
				...
				$output .= $counter > 0 ? sprintf( '<span class="cs-sep">%s</span>', esc_html__( 'and', 'newsblock' ) ) : '';
				...
				$output .= sprintf(
					'<a class="cs-meta-author-inner url fn n" href="%1$s" title="%2$s">%4$s<span class="cs-by">' . esc_html__( 'by', 'newsblock' ) . '</span><span class="cs-author">%3$s</span></a>',
					esc_url( $posts_url ),
					/* translators: %s: author name */
					esc_attr( sprintf( __( 'View all posts by %s', 'newsblock' ), $display_name ) ),
					$display_name,
					$author_avatar
				);
...
if ( ! function_exists( 'csco_get_meta_comments' ) ) {
	...
	function csco_get_meta_comments( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( true === $compact ) {
			...
			ob_start();
			comments_popup_link( esc_html__( 'No comments', 'newsblock' ), esc_html__( 'One comment', 'newsblock' ), '% ' . esc_html__( 'comments', 'newsblock' ), 'comments-link', '' );
			$output .= ob_get_clean();
		}
...
if ( ! function_exists( 'csco_get_meta_reading_time' ) ) {
	...
	function csco_get_meta_reading_time( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( true === $compact ) {
			$output .= intval( $reading_time ) . ' ' . esc_html__( 'minute read', 'newsblock' );
		} else {
			/* translators: %s number of minutes */
			$output .= esc_html( sprintf( _n( '%s minute read', '%s minute read', $reading_time, 'newsblock' ), $reading_time ) );
		}
...
if ( ! function_exists( 'csco_get_meta_views' ) ) {
	...
	function csco_get_meta_views( $tag = 'div', $compact = false, $settings = array() ) {
		...
		if ( true === $compact ) {
			...
			if ( $views > 1000 ) {
				$output .= $views_rounded . ' ' . esc_html__( 'views', 'newsblock' );
			} else {
				/* translators: %s number of post views */
				$output .= esc_html( sprintf( _n( '%s view', '%s views', $views, 'newsblock' ), $views ) );
			}
		}
			...
			<div class="cs-meta-share-total">
				...
				<span class="cs-total-number">
					...
					if ( true === $compact ) {
						...
						if ( $shares > 1000 ) {
							echo esc_html( $shares_rounded ) . ' ' . esc_html__( 'shares', 'newsblock' );
						} else {
							/* translators: %s number of post views */
							echo esc_html( sprintf( _n( '%s share', '%s shares', $shares, 'newsblock' ), $shares ) );
						}
					}
					?>
				</span>
