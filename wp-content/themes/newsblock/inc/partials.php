	...
	function csco_page_pagination() {
		...
		wp_link_pages(
			array(
				...
				'separator'        => ' ',
				'nextpagelink'     => esc_html__( 'Next page', 'newsblock' ),
				'previouspagelink' => esc_html__( 'Previous page', 'newsblock' ),
			)
		);
...
if ( ! function_exists( 'csco_entry_header_large' ) ) {
	...
	function csco_entry_header_large() {
		...
		if ( ! ( 'large' === csco_get_page_header_type() ||
			'large_overlay' === csco_get_page_header_type() ) ) {
			return;
		}
...
if ( ! function_exists( 'csco_entry_prev_nex' ) ) {
	/**
	 * Entry Prev Next
	 */
	function csco_entry_prev_nex() {
		if ( ! is_singular( 'post' ) ) {
			return;
		}
		if ( false === get_theme_mod( 'post_prev_next', false ) ) {
			return;
		}
